...
release workflow 使用 `scripts/release/package.sh` 生成 macOS / Linux 包，使用
...
并拒绝缺失库、构建目录依赖和 `/usr/local` 依赖；Windows 包复制运行所需 DLL，并记录
`dumpbin /dependents` 清单。所有平台都会在上传前解包、校验 checksum，并运行 `mradm --version` /
