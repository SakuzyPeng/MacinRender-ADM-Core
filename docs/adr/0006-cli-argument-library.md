...
> 状态：已接受  
> 日期：2026-05-17  
> 适用范围：`src/adm_cli/`、未来基于核心库的命令行工具（例如 `tools/adm/*`）。GUI（图形用户界面）、Rust 工具链、C ABI（应用二进制接口）调用方不受本 ADR 约束。
...
- 子命令（subcommand）支持完整，能表达 `adm render`、`adm probe`、`adm batch`、`adm diagnose` 这类多动词 CLI 形态。
