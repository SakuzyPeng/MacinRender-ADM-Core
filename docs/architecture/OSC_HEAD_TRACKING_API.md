...
俯仰到达 ±90° 附近时，Euler 的 yaw／roll 不再能独立确定；接收器选取 roll=0 并保留合并朝向，保证输出旋转与四元数等价。
...
info／status 分别按来源与实例保留最新消息和排序记录，每类最多缓存 17 个实例。
候选遥测不能挤出当前实例的记录或刷新其心跳；新实例收到首份有效姿态后才采用该实例的缓存。
...
- 已知源停止、info 或 status 上报的参考变化会使已有姿态失效；参考变化不依赖两类遥测同时到达，较旧的 stopped 状态不能覆盖后来的新采样。
