...
| 能力 | 当前状态 | 建议定位 |
|---|---|---|
| **HRTF / SOFA binauraliser** | ✅ 已有独立 `binaural` 后端：SAF 内置 KEMAR HRTF，836 方向，256-tap FIR，固定 48 kHz，输出 2ch binaural；⚠️ 外部 SOFA 加载未实现 | 后续可扩展 SOFA/HRTF 数据集选择、采样率匹配和更完整距离策略 |
...
Objects / DirectSpeakers → 位置解析 → SAF 内置 KEMAR HRTF（836 方向）→ OLA 分块卷积 → 固定 2ch binaural 输出。
渲染器会忽略用户请求的扬声器布局；CAF/APAC 写入时使用 CoreAudio `Binaural` layout tag，
...
| ~~HRTF binauraliser 基础后端~~ | ~~已实现：SAF KEMAR HRTF，Objects/DirectSpeakers，固定 48 kHz，CAF/APAC 标记为 CoreAudio Binaural~~ |
| SOFA binauraliser | 外部 HRTF 数据集加载、采样率匹配和选择策略尚未实现 |
