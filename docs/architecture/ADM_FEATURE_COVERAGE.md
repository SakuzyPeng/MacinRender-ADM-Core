...
HOA encode 路径同样按 `√(1-d)` / `√d` 拆分 direct 与 diffuse；direct 保持对象
SH 编码，diffuse 按 divergence 槽位保留 HOA 系数，并通过逐 HOA 系数 32-tap 短延迟去相关后输出。
...
- **Objects→HOA encode**：读 Objects 块位置、gain、diffuse、width/height/depth 和 objectDivergence，输出 16ch HOA3 ACN/SN3D。WAV 通过 AmbiX `ambi` chunk 标记，CAF / APAC 使用 CoreAudio `HOA_ACN_SN3D` layout tag。
...
| diffuse 参数 | ✅ 已实现：direct / diffuse 能量拆分，diffuse 按 divergence 槽位保留 HOA 系数并做逐系数 32-tap 去相关 |
| width / height / depth | ✅ 已实现：构建 gain matrix 时烘焙为对称 angular extent cloud；采样按内/外环半径加权，W 保持能量基准，高阶方向性随 extent 增大衰减 |
| objectDivergence | ✅ 已实现：direct 和 diffuse 分量均展开为左右/中心虚拟源并聚合为单个 HOA block；diffuse 每个 divergence 槽位使用独立 decorrelator |
...
| 能力 | 当前状态 | 建议定位 |
|---|---|---|
| **HRTF / SOFA binauraliser** | ✅ 已有独立 `saf-binaural` 后端：默认 SAF 内置 KEMAR HRTF，支持 `--sofa` 加载用户 FIR SOFA HRIR；Objects 已接入 channelLock / objectDivergence / extent / diffuse，输出 2ch binaural | 后续可扩展 HOA 输入、更多 HRTF 数据集选择、距离策略和房间响应 |
...
`saf-binaural` 已从普通立体声别名升级为独立双耳后端。当前实现路径为：
...
`saf-binaural`，显式请求 `--renderer ear|saf --output-layout stereo/0+2+0` 会返回
...
EAR diffuse bus 已在 M4 中实现（见注②）：`designDecorrelators()` FIR 去相关 + `decorrelatorCompensationDelay()` direct 延迟补偿。HOA encode 不绑定扬声器布局；它将 diffuse 能量保留在 HOA 系数域，并按 objectDivergence 的 left/center/right 槽位使用独立 32-tap 短延迟 decorrelator。VBAP 的 MDAP spread 对应 extent 几何扩散，与 ADM `diffuse` 语义无关，暂不计划实现 VBAP 侧去相关。
...
| ~~Objects 动态元数据去拉链~~ | ~~已实现：`RenderOptions.object_smoothing_frames`（默认 0，CLI `--object-smoothing-frames`），覆盖 VBAP/EAR/HOA encode/binaural；DirectSpeakers 保持物理声道语义~~ |
...
## 11. Semantic policy 渲染控制覆盖

`--semantic-policy <json>`（schema `mradm.semantic-policy.v1`）在 import 之后、渲染之前对
`AdmScene` 做覆盖；不修改源 AXML，仅影响本次渲染。实现见 `src/adm_core/semantic_policy.cpp`，
能力测试见 `tests/unit/semantic_policy_test.cpp`。`global` 作用于全部内容，`objects[]` 是按规则
匹配的覆盖（命中即合并；DirectSpeakers 规则按序独立应用，不合并过滤器）。

### 匹配维度（规则内 OR 组合）

| 维度 | 作用对象 | 说明 |
|---|---|---|
| `id` / `name` / `name_glob` / `track_uid` | Objects / DS（HOA 仅 `id`==object_id） | 标识匹配，大小写不敏感；glob 通配 |
| `all` | Objects / DS / HOA | 全部 |
| `importance_min` / `importance_max` / `dialogue_id` | Objects / DS | 按 `SceneObject` 字段（有值时）|
| `content` / `programme` | Objects / DS | 经 content→object / programme→content→object 成员表 |
| `pack_format` | HOA | == `SceneHOATracks.pack_format_id` |
