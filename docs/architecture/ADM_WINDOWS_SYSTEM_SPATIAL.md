...
`22.2`（`9+10+3`）当前不发布为 Windows 系统空间音频监听布局：按原生 22.2 需要 23 个 Windows spatial objects（24 路输入、双 LFE 合并为一个 LFE 对象），在 windows-testbox 真机播放时会触发 Windows 空间化器的 `CompPkgSup.dll` 崩溃。实现不把 22.2 折叠成更小对象预算来冒充支持；如果平台不能原生承载，就按 unsupported 处理。
...
| `9+10+3`（22.2，未发布） | 原生序为 M+060 M-060 M+000 LFE1 M+135 M-135 M+030 M-030 M+180 LFE2 M+090 M-090 U+045 U-045 U+000 T+000 U+135 U-135 U+090 U-090 U+180 B+000 B+045 B-045；该映射需要 23 个 spatial objects，当前 Windows 空间化器会在播放时崩溃，因此不暴露为支持布局 |
...
- 唯一差别是**动态对象容量**：Sonic > DTS > Dolby，但三者都足够当前发布布局（9.1.6 需要 4 个；数字对上官方“较早 Windows 版本”资源表；新 servicing 把三者统一到 128；LFE 不计入额度）。22.2 虽只需 8 个动态对象，但总对象数 23 会触发实测空间化器崩溃，不能仅用动态容量判断支持。
