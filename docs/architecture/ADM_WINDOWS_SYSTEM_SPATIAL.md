...
支持的监听布局：`9.1.6`、`7.1.4`（`4+7+0`）、`5.1.4`（`4+5+0`）、`5.1`（`0+5+0`）。

`22.2`（`9+10+3`）当前不发布为 Windows 系统空间音频监听布局：按原生 22.2 需要 23 个 Windows spatial objects（24 路输入、双 LFE 合并为一个 LFE 对象）。Microsoft Spatial Sound 文档只承诺 8.1.4.4 静态床、格式相关的动态对象预算，以及由当前端点/空间化器决定的资源上限；它没有把 22.2 原生床作为可用监听布局承诺。windows-testbox 真机测试中，这个 23-object 原生 22.2 组合会在播放阶段触发 Windows 空间化器的 `CompPkgSup.dll` 崩溃。实现不把 22.2 折叠成更小对象预算来冒充支持；如果平台不能原生承载，就按 unsupported 处理。
...
## 3. 架构：事件驱动 pull + 静态/动态对象床

ISpatialAudioClient 是**对象制**而非声道制:不存在「把 24 声道塞进 N 声道端点」。我们把监听床拆成一组**单声道 spatial audio object** 提交，空间引擎再把这组对象 HRTF 渲染到端点实际格式（耳机 2ch）。

对象分两类：

- **静态对象**：Windows `AudioObjectType_*` 已命名的位置（Front/Side/Back、LFE、Top、Bottom、BackCenter）走 `StaticObjectTypeMask`。
- **动态对象**：Windows 静态枚举没有的位置（9.1.6 的 wide/top-middle）走 `AudioObjectType_Dynamic`，激活后用 `SetPosition(x,y,z)` 固定在对应 BS.2051 方位。22.2 的原生动态映射保留在代码注释/表中，但不发布到可选布局。
...
每个 spatial object 格式固定 `WAVE_FORMAT_IEEE_FLOAT / 1ch / 48000Hz / 32bit`，`IsAudioObjectFormatSupported` 校验后激活。实测三个空间化器**只接受这一种对象格式**（见 §6），监听本就跑 48k，因而总是匹配；若不匹配则 `start()` 返回 `unsupported`。

## 4. 布局 → AudioObjectType 映射（`windows_layouts.h`）

每个布局给出按 **project 交织声道序** 排列的 `ChannelRoute` 列表；声道序对齐 `src/adm_engine/layout_table.cpp` 的序（监听 sink 收到的就是这个序，与 CAF/APAC writer 一致）。
...
| `9.1.6` | L R C LFE Ls Rs Rls Rrs **Lw Rw** Vhl Vhr **Ltm Rtm** Ltr Rtr；加粗 4 个位置为动态对象 |
| `9+10+3`（22.2，未发布） | 原生序为 M+060 M-060 M+000 LFE1 M+135 M-135 M+030 M-030 M+180 LFE2 M+090 M-090 U+045 U-045 U+000 T+000 U+135 U-135 U+090 U-090 U+180 B+000 B+045 B-045；该映射需要 23 个 spatial objects，但不是 Microsoft Spatial Sound 文档承诺的原生监听床；windows-testbox 当前空间化器在播放该组合时崩溃，因此不暴露为支持布局 |

要点：7.1.4 里 project 的 `Ls/Rs` 是**侧环绕**（另有 `Rls/Rrs` 后环绕）→ 映射到 `SideLeft/SideRight` + `BackLeft/BackRight`；5.1/5.1.4 无独立后环绕，其唯一环绕对映射到后位（与 WAVEFORMATEXTENSIBLE 5.1 mask 约定一致）。映射用**具名常量**，不依赖手写位值。`static_object_mask()` 只把静态路由按位或得到 `StaticObjectTypeMask`；`dynamic_object_count()` 申请动态对象预算。22.2 不做折叠降级；原生 23-object 路径若不可用就失败。
...
在 windows-testbox（设备 `[REDACTED-DEVICE]`）逐个切换 Windows 声音设置的空间格式，经 `ISpatialAudioClient` 实测，并与微软文档核对：
...
- 三者**静态床、对象格式、缓冲帧数完全相同**；本 sink 的静态/动态组合床三者通用。
- 原生静态床是 **8.1.4.4（17 位）**：环绕 8（含 BackCenter）+ LFE 1 + 顶 4（TpFL/FR/BL/BR）+ **底 4（BottomFL/FR/BL/BR）**。5.1/5.1.4/7.1.4 是它的子集。
- 唯一差别是**动态对象容量**：Sonic > DTS > Dolby，但三者都足够当前发布布局（9.1.6 需要 4 个；数字对上官方“较早 Windows 版本”资源表；新 servicing 把三者统一到 128；LFE 不计入额度）。22.2 虽只需 8 个动态对象、总对象数也不一定越过所有官方表格里的数字上限，但官方没有承诺 22.2 原生床；实测还会在播放阶段触发空间化器崩溃，因此不能仅用动态容量判断支持。
...
- 9.1.6 中静态枚举没有的位置依赖动态对象；若某个未来空间化器动态对象预算不足，`start()` 会在激活阶段返回 `unsupported`，而不是播放中静默丢声道。
- 22.2 当前在 Windows system-spatial 下按 unsupported 处理；不做 16-object 或 7.1.4 等折叠近似。
- 无 OS 头追（Windows 平台限制）。
- Debug 构建的 capi bundle 为 `/MDd`，依赖 VS 调试 CRT，仅供本机验证，非分发件。

## 10. 验证

- 编译：CI `windows-debug` job + windows-testbox（规范配方）。
- 回归：windows-testbox 全量 ctest 23/23。
- 端到端：GUI `--selftest` 确认 `system_spatial_layouts` = `[7.1.4, 5.1.4, 5.1, 9.1.6]` 及逐声道标签。
...
2. **源端头追**（OpenTrack 等）：在床渲染阶段按 `listener_orientation` 旋转，为 Windows 补头追。
3. **真机听验矩阵**：继续覆盖 9.1.6 的动态对象方位，以及 Dolby/Sonic/DTS 三空间化器切换恢复。22.2 只有在能原生 23-object 播放且不触发系统空间化器崩溃时再重新发布。
