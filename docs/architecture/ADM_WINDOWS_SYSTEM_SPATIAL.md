...
`22.2`（`9+10+3`）当前不发布为 Windows 系统空间音频监听布局：按原生 22.2 需要 23 个 Windows spatial objects（24 路输入、双 LFE 合并为一个 LFE 对象），在 win-pc 真机播放时会触发 Windows 空间化器的 `CompPkgSup.dll` 崩溃。实现不把 22.2 折叠成更小对象预算来冒充支持；如果平台不能原生承载，就按 unsupported 处理。
