...
- **动态对象**：Windows 静态枚举没有的位置（9.1.6 的 wide/top-middle；22.2 的 front-wide、top-center/top-side/top-back-center、bottom-center）走 `AudioObjectType_Dynamic`，激活后用 `SetPosition(x,y,z)` 固定在对应 BS.2051 方位。
...
| `9+10+3`（22.2） | M+060 M-060 M+000 LFE1 M+135 M-135 M+030 M-030 M+180 LFE2 M+090 M-090 U+045 U-045 U+000 T+000 U+135 U-135 U+090 U-090 U+180 B+000 B+045 B-045；Windows 无静态名的位置走动态对象，LFE1/LFE2 合并到单个 LFE 静态对象 |

要点：7.1.4 里 project 的 `Ls/Rs` 是**侧环绕**（另有 `Rls/Rrs` 后环绕）→ 映射到 `SideLeft/SideRight` + `BackLeft/BackRight`；5.1/5.1.4 无独立后环绕，其唯一环绕对映射到后位（与 WAVEFORMATEXTENSIBLE 5.1 mask 约定一致）。映射用**具名常量**，不依赖手写位值。`static_object_mask()` 只把静态路由按位或得到 `StaticObjectTypeMask`；`dynamic_object_count()` 申请动态对象预算；`object_count()` 允许 22.2 的 24 个输入声道折成 23 个 spatial objects。
...
- 原生静态床是 **8.1.4.4（17 位）**：环绕 8（含 BackCenter）+ LFE 1 + 顶 4（TpFL/FR/BL/BR）+ **底 4（BottomFL/FR/BL/BR）**。5.1/5.1.4/7.1.4 是它的子集；22.2 额外使用 BackCenter 与 BottomFrontLeft/Right，剩余无静态名的位置走动态对象。
- 唯一差别是**动态对象容量**：Sonic > DTS > Dolby，但三者都足够当前布局（9.1.6 需要 4 个，22.2 需要 8 个；数字对上官方“较早 Windows 版本”资源表；新 servicing 把三者统一到 128；LFE 不计入额度）。
...
- 9.1.6 / 22.2 中静态枚举没有的位置依赖动态对象；若某个未来空间化器动态对象预算不足，`start()` 会在激活阶段返回 `unsupported`，而不是播放中静默丢声道。
- 22.2 双 LFE 在 Windows 只能监听为单个 LFE 对象；这是平台槽位限制，不等同于离线 24ch 文件里的两个独立 LFE 声道。
