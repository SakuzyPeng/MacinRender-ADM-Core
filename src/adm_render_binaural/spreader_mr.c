    ...
    if ((nSamples == SPREADER_FRAME_SIZE) && (pData->codecStatus == CODEC_STATUS_INITIALISED)) {
        ...
        for (src = 0; src < nSources; src++) {
             ...
             * precompute): avoids iterating all nGrid directions every frame for static sources. */
            {
                ...
                int needs_rebuild = !pData->cone_valid[src] || az_s != pData->cone_cached_az[src] ||
                                    el_s != pData->cone_cached_el[src] || sp_s != pData->cone_cached_spread[src];
                if (needs_rebuild) {
            ...
            case SPREADER_MODE_OM:
                for (band = 0; band < HYBRID_BANDS; band++) {
                    ...
                    for (k = 0; k < nc; k++) {
                        ng = cng[k];
                        for (q = 0; q < Q; q++)
                            pData->_H_tmp[q] = ccaddf(pData->_H_tmp[q],
