...
constexpr uint32_t kTagName          = 0x4487U;
constexpr uint32_t kTagString        = 0x4484U;

using MkaBuf = std::vector<uint8_t>;

static void mka_write_id(MkaBuf& b, uint32_t id) {
    if      (id <= 0xFFU)       { b.push_back(static_cast<uint8_t>(id)); }
    else if (id <= 0xFFFFU)     { b.push_back(static_cast<uint8_t>(id >> 8U)); b.push_back(static_cast<uint8_t>(id)); }
    else if (id <= 0xFFFFFFU)   { b.push_back(static_cast<uint8_t>(id >> 16U)); b.push_back(static_cast<uint8_t>(id >> 8U)); b.push_back(static_cast<uint8_t>(id)); }
    else                         { b.push_back(static_cast<uint8_t>(id >> 24U)); b.push_back(static_cast<uint8_t>(id >> 16U)); b.push_back(static_cast<uint8_t>(id >> 8U)); b.push_back(static_cast<uint8_t>(id)); }
}

static void mka_write_vint(MkaBuf& b, uint64_t v) {
    if      (v < 0x7FULL)          { b.push_back(static_cast<uint8_t>(v | 0x80U)); }
    else if (v < 0x3FFFULL)        { b.push_back(static_cast<uint8_t>((v >> 8U) | 0x40U)); b.push_back(static_cast<uint8_t>(v)); }
    else if (v < 0x1FFFFFULL)      { b.push_back(static_cast<uint8_t>((v >> 16U) | 0x20U)); b.push_back(static_cast<uint8_t>(v >> 8U)); b.push_back(static_cast<uint8_t>(v)); }
    else if (v < 0x0FFFFFFFULL)    { b.push_back(static_cast<uint8_t>((v >> 24U) | 0x10U)); b.push_back(static_cast<uint8_t>(v >> 16U)); b.push_back(static_cast<uint8_t>(v >> 8U)); b.push_back(static_cast<uint8_t>(v)); }
    else if (v < 0x07FFFFFFFFULL)  { b.push_back(static_cast<uint8_t>((v >> 32U) | 0x08U)); b.push_back(static_cast<uint8_t>(v >> 24U)); b.push_back(static_cast<uint8_t>(v >> 16U)); b.push_back(static_cast<uint8_t>(v >> 8U)); b.push_back(static_cast<uint8_t>(v)); }
    else { b.push_back(0x01U); for (int i = 7; i >= 0; --i) { b.push_back(static_cast<uint8_t>(v >> (static_cast<unsigned>(i) * 8U))); } }
}

static void mka_write_unknown_size(MkaBuf& b) {
    b.push_back(0x01U);
    for (int i = 0; i < 7; ++i) { b.push_back(0xFFU); }
}

static void mka_write_uint_be(MkaBuf& b, uint64_t v, int width) {
    for (int i = width - 1; i >= 0; --i) { b.push_back(static_cast<uint8_t>(v >> (static_cast<unsigned>(i) * 8U))); }
}
...
static void mka_write_file(FILE* f, const MkaBuf& b) {
    std::fwrite(b.data(), 1, b.size(), f);
}

static std::vector<uint8_t> build_opus_head(uint32_t channels, int nb_streams,
                                             int nb_coupled, const uint8_t* mapping,
                                             uint16_t preskip, uint32_t input_sample_rate,
                                             uint8_t family) {
    ...
    h.push_back(0U); h.push_back(0U);  // output gain = 0
    h.push_back(family);
    if (family != 0U) {
        h.push_back(static_cast<uint8_t>(nb_streams));
        h.push_back(static_cast<uint8_t>(nb_coupled));
        for (uint32_t i = 0; i < channels; ++i) { h.push_back(mapping[i]); }
    }
    return h;
}

} // anonymous namespace (MKA helpers)
...
struct FloatOpusMkaWriter::Impl {
    ...
    void open_cluster(uint64_t ts_ms);
    void encode_and_write(uint64_t frame_ts_ms);
    ~Impl();
};
...
FloatOpusMkaWriter::Impl::~Impl() {
    if (!ibuf.empty() && file != nullptr) {
        ...
        encode_and_write(frame_ms);
        ++total_frames;
    }
    if (file != nullptr && duration_patch_offset >= 0) {
        const double duration_ms = static_cast<double>(total_frames) *
            (static_cast<double>(k_frame_size) / 48000.0) * 1000.0;
        MkaBuf tmp;
        mka_write_float64_be(tmp, duration_ms);
        std::fseek(file, duration_patch_offset, SEEK_SET);
        std::fwrite(tmp.data(), 1, tmp.size(), file);
        std::fclose(file);
        file = nullptr;
    }
    if (encoder != nullptr) {
...
void FloatOpusMkaWriter::Impl::open_cluster(uint64_t ts_ms) {
    cluster_ts_ms = ts_ms;
    cluster_open = true;
    MkaBuf hdr;
    mka_write_id(hdr, kCluster);
    mka_write_unknown_size(hdr);
    mka_write_file(file, hdr);
    MkaBuf tb;
    mka_elem_uint(tb, kTimestamp, ts_ms);
    mka_write_file(file, tb);
}

void FloatOpusMkaWriter::Impl::encode_and_write(uint64_t frame_ts_ms) {
    if (!cluster_open || frame_ts_ms - cluster_ts_ms > 5000U) {
        open_cluster(frame_ts_ms);
    }
    const int ret = opus_multistream_encode_float(
        encoder, ibuf.data(), k_frame_size,
        pbuf.data(), static_cast<opus_int32>(pbuf.size()));
    if (ret <= 0) { return; }

    const auto rel_ms = static_cast<int16_t>(
        static_cast<int64_t>(frame_ts_ms) - static_cast<int64_t>(cluster_ts_ms));
    ...
    mka_write_file(file, block_elem);
...
FloatOpusMkaWriter::open(const std::string& path, uint32_t channels, uint32_t sample_rate) {
    ...
    if (channels < 1U || channels > 255U) {
        return make_error(ErrorCode::unsupported,
            fmt::format("Opus supports 1-255 channels, got {}", channels),
            "path=" + path);
    }
    ...
    int error{};
    impl->encoder = opus_multistream_surround_encoder_create(
        48000, static_cast<int>(channels), family,
        &impl->nb_streams, &impl->nb_coupled, impl->mapping,
    ...
    const int32_t bitrate = (channels <= 2U)
        ? 128000
        : static_cast<int32_t>(channels) * 64000;
    opus_multistream_encoder_ctl(impl->encoder, OPUS_SET_VBR(1));
    opus_multistream_encoder_ctl(impl->encoder, OPUS_SET_VBR_CONSTRAINT(0));
    opus_multistream_encoder_ctl(impl->encoder, OPUS_SET_BITRATE(bitrate));

    int32_t preskip{};
    opus_multistream_encoder_ctl(impl->encoder, OPUS_GET_LOOKAHEAD(&preskip));
    impl->preskip = preskip;

    impl->pbuf.resize(static_cast<std::size_t>(impl->nb_streams) * 1275);

    const auto opus_head = build_opus_head(
        channels, impl->nb_streams, impl->nb_coupled, impl->mapping,
        static_cast<uint16_t>(preskip), 48000U, static_cast<uint8_t>(family));
    ...
    // ── Info ─────────────────────────────────────────────────────────────────
    {
        ...
        const std::size_t sz_bytes =
            (info_content_size < 0x7FULL)     ? 1U :
            (info_content_size < 0x3FFFULL)   ? 2U :
            (info_content_size < 0x1FFFFFULL) ? 3U : 4U;
        ...
        impl->duration_patch_offset = static_cast<long>(
            info_elem_start + k_info_id_bytes + sz_bytes + 10U);
    ...
    // ── Tracks ────────────────────────────────────────────────────────────────
    {
        ...
        const uint64_t codec_delay_ns =
            (static_cast<uint64_t>(preskip) * 1000000000ULL) / 48000ULL;
    ...
    mka_write_file(f, ebml);

    impl->file = f;
    impl->open_cluster(0U);
...
uint64_t FloatOpusMkaWriter::write(const float* samples, uint64_t frame_count) {
    ...
    while (frames_consumed < frame_count) {
        const std::size_t needed =
            static_cast<std::size_t>(Impl::k_frame_size) * ch - impl_->ibuf.size();
        const std::size_t avail =
            static_cast<std::size_t>(frame_count - frames_consumed) * ch;
        ...
        if (impl_->ibuf.size() == static_cast<std::size_t>(Impl::k_frame_size) * ch) {
            ...
            impl_->encode_and_write(frame_ms);
            ++impl_->total_frames;
        }
    }
    return frame_count;
...
Result<void> convert_to_opus_mka(const std::string& src_path,
    ...
    if (!reader_res) { return tl::unexpected{reader_res.error()}; }
    auto& reader = *reader_res;
    ...
    if (!writer_res) { return tl::unexpected{writer_res.error()}; }
    auto& writer = *writer_res;
    ...
    while (left > 0) {
        ...
        if (writer.write(buf.data(), got) != got) {
            return make_error(ErrorCode::io_error,
                "short write in convert_to_opus_mka", "path=" + mka_path);
        }
        left -= got;
    }
    if (left != 0) {
        return make_error(ErrorCode::io_error,
            "short read in convert_to_opus_mka", "src=" + src_path);
    }
    (void)layout_id;
    return {};
}
...
// write_mka_metadata: append a Tags element to an existing MKA file.
Result<void> write_mka_metadata(const std::string& path, const MetadataFields& meta) {
    FILE* f = std::fopen(path.c_str(), "ab");
    if (f == nullptr) {
        return make_error(ErrorCode::io_error, "cannot open MKA for metadata append", "path=" + path);
    }
    ...
    std::fwrite(tags.data(), 1, tags.size(), f);
    std::fclose(f);
    return {};
}
