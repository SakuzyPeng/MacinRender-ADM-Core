...
absl::Status CancelledStatus() {
    return absl::CancelledError("IAMF encode cancelled");
}

absl::Status CheckCancelled(const MrIamfCancelProbe& cancel_probe) {
...
absl::Status PartitionParameterMetadata(UserMetadata& user_metadata) {
    ...
    if (user_metadata.ia_sequence_header_metadata().empty() || user_metadata.codec_config_metadata().empty()) {
        return absl::InvalidArgumentError("Determining the partition duration requires at least one "
                                          "`ia_sequence_header_metadata` and one `codec_config_metadata`");
    }
    std::list<ParameterBlockObuMetadata> partitioned_parameter_blocks;
    RETURN_IF_NOT_OK(
        ParameterBlockPartitioner::FindPartitionDuration(user_metadata.ia_sequence_header_metadata(0).primary_profile(),
                                                         user_metadata.codec_config_metadata(0),
                                                         partition_duration));
    for (const auto& parameter_block_metadata : user_metadata.parameter_block_metadata()) {
        RETURN_IF_NOT_OK(ParameterBlockPartitioner::PartitionFrameAligned(
            partition_duration, parameter_block_metadata, partitioned_parameter_blocks));
    }
...
typedef absl::flat_hash_map<InternalTimestamp, std::list<ParameterBlockObuMetadata>> TimeParameterBlockMetadataMap;
absl::Status OrganizeParameterBlockMetadata(
    const google::protobuf::RepeatedPtrField<ParameterBlockObuMetadata>& parameter_block_metadata,
    TimeParameterBlockMetadataMap& time_parameter_block_metadata) {
    for (const auto& metadata : parameter_block_metadata) {
        time_parameter_block_metadata[metadata.start_timestamp()].push_back(metadata);
    }
...
absl::Status
CollectLabeledSamplesForAudioElements(const DescriptorObus::AudioElementsById& audio_elements,
    ...
    for (const auto& [audio_element_id, unused_audio_element] : audio_elements) {
        RETURN_IF_NOT_OK(wav_sample_provider.ReadFrames(
            audio_element_id, id_to_labeled_samples[audio_element_id], no_more_real_samples));
    }
    return absl::OkStatus();
}

absl::Status CreateOutputDirectory(const std::string& output_directory) {
    if (output_directory.empty() || std::filesystem::is_directory(output_directory) ||
    ...
    if (!std::filesystem::create_directories(output_directory, error_code)) {
        return absl::UnknownError(absl::StrCat("Failed to create output directory = ", output_directory));
    }

    return absl::OkStatus();
}

absl::Status LabeledSamplesToAudioElementData(const LabelSamplesMap& labeled_samples,
    ...
    for (const auto& [channel_label, samples] : labeled_samples) {
        ...
        if (!channel_label_message.SerializeToString(&serialized_channel_label_message)) {
            return absl::InternalError("Failed to serialize a `ChannelLabelMessage` protocol buffer.");
        }

        audio_element_data[serialized_channel_label_message] = absl::Span<const double>(samples);
    }
    return absl::OkStatus();
}
...
absl::Status GenerateTemporalUnitObus(const UserMetadata& user_metadata,
    ...
    auto wav_sample_provider = WavSampleProvider::Create(
        user_metadata.audio_frame_metadata(), input_wav_directory, iamf_encoder.GetAudioElements());
    ...
    TimeParameterBlockMetadataMap time_parameter_block_metadata;
    RETURN_IF_NOT_OK(
        OrganizeParameterBlockMetadata(user_metadata.parameter_block_metadata(), time_parameter_block_metadata));
    ...
    while (iamf_encoder.GeneratingTemporalUnits()) {
        RETURN_IF_NOT_OK(CheckCancelled(cancel_probe));
        ...
        RETURN_IF_NOT_OK(CollectLabeledSamplesForAudioElements(
            iamf_encoder.GetAudioElements(), *wav_sample_provider, id_to_labeled_samples, no_more_real_samples));
        RETURN_IF_NOT_OK(CheckCancelled(cancel_probe));

        // Adapt the audio samples into the expected format for the encoder.
        for (const auto& [audio_element_id, labeled_samples] : id_to_labeled_samples) {
            RETURN_IF_NOT_OK(LabeledSamplesToAudioElementData(
                labeled_samples, temporal_unit_data.audio_element_id_to_data[audio_element_id]));
        }
        // Fill in this temporal unit's parameter block metadata.
        for (const auto& metadata : time_parameter_block_metadata[input_timestamp]) {
            std::string serialized_metadata;
            if (!metadata.SerializeToString(&serialized_metadata)) {
                return absl::InternalError("Failed to serialize parameter block metadata.");
            }
            temporal_unit_data.parameter_block_id_to_metadata[metadata.parameter_id()] = serialized_metadata;
        }
        ...
        RETURN_IF_NOT_OK(iamf_encoder.OutputTemporalUnit(unused_temporal_unit_obus));
        RETURN_IF_NOT_OK(CheckCancelled(cancel_probe));
    }
...
absl::Status TestMainCancellable(const UserMetadata& input_user_metadata,
                                 const std::string& input_wav_directory,
    ...
    if (user_metadata.test_vector_metadata().partition_mix_gain_parameter_blocks()) {
    ...
    const std::string output_wav_file_prefix =
        (std::filesystem::path(output_iamf_directory) / user_metadata.test_vector_metadata().file_name_prefix())
            .string();
    ABSL_LOG(INFO) << "output_wav_file_prefix = " << output_wav_file_prefix;
    RenderingMixPresentationFinalizer::SampleProcessorFactory sample_processor_factory =
        [output_wav_file_prefix](DecodedUleb128 mix_presentation_id,
                                 ...
                                 int sample_rate,
                                 int bit_depth,
                                 size_t max_input_samples_per_frame) -> std::unique_ptr<SampleProcessorBase> {
        // Generate a unique filename for each layout of each mix presentation.
        const auto wav_path = absl::StrCat(output_wav_file_prefix,
                                           "_rendered_id_",
                                           mix_presentation_id,
                                           "_sub_mix_",
                                           sub_mix_index,
                                           ...
                                           layout_index,
                                           ".wav");
        return WavWriter::Create(wav_path, num_channels, sample_rate, bit_depth, max_input_samples_per_frame);
    };

    // Apply the bit depth override.
    ApplyOutputAudioFormatToSampleProcessorFactory(
        user_metadata.encoder_control_metadata().output_rendered_file_format(), sample_processor_factory);
    ...
    IamfEncoder::ObuSequencerFactory obu_sequencer_factory = [&]() {
        return CreateObuSequencers(user_metadata,
                                   output_iamf_directory,
                                   user_metadata.temporal_delimiter_metadata().enable_temporal_delimiters());
    };

    auto iamf_encoder = IamfEncoder::Create(user_metadata,
                                            CreateRendererFactory().get(),
    ...
    // finalized.
    RETURN_IF_NOT_OK(GenerateTemporalUnitObus(user_metadata, input_wav_directory, **iamf_encoder, cancel_probe));
