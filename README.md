...
FLAC 当前最多支持 8 声道，因此不能用于 `5.1.4`、`7.1.4`、`9.1.6`、`22.2` 等超过 8 声道的布局；这些布局请使用 WAV、CAF、APAC 或 Opus MKA。Opus MKA 对 `5.1` / `7.1` 会按 Opus/Vorbis 声道顺序编码；更高阶离散布局仅作为多流编码交付，播放器不一定能按空间布局直接回放。

HOA 直接回放目前只确认 macOS 上的 CAF 与 APAC 可行。WAV 会写 AmbiX `ambi` chunk，适合支持 AmbiX 的工具链；Opus MKA 会写 ambisonics mapping，但常见播放器兼容性有限，不作为通用直接监听格式。已验证 VLC 4.0 可回放 Opus HOA3；播放时需在音频选项中将 mix node 从 `original: ambisonics` 改为 `binaural`，由 VLC 解码到双耳输出。
