...
FLAC 当前最多支持 8 声道，因此不能用于 `5.1.4`、`7.1.4`、`9.1.6`、`22.2` 等超过 8 声道的布局；这些布局请使用 WAV、CAF、APAC 或 Opus MKA。Opus MKA 对 `5.1` / `7.1` 会按 Opus/Vorbis 声道顺序编码；更高阶离散布局仅作为多流编码交付。实测 PotPlayer 可正常回放 Opus MKA 的离散声道内容，虽然会下混到 8ch，但主观听感仍较完整；部分具备系统 Opus decoder（如 `c2.android.opus.decoder` / `OMX.google.opus.decoder`）的 Android 设备可回放最高 `9.1.6` 的 Opus MKA。上述兼容性不代表播放器能保留完整空间布局语义。

macOS 可直接回放本项目写出的 CAF 以及 APAC `.mp4/.m4a` 空间音频文件。WAV 多声道直放尚未详细测试；WAV HOA3 虽会写 AmbiX `ambi` chunk，但 macOS 不能直接按 HOA 回放，更适合支持 AmbiX 的工具链。Opus MKA 会写 ambisonics mapping，但常见播放器兼容性有限，不作为通用直接监听格式。已验证 VLC 4.0 可回放 Opus HOA3；播放时需在音频选项中将 mix node 从 `original: ambisonics` 改为 `binaural`，由 VLC 解码到双耳输出。
