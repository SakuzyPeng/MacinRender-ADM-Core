...
| Opus | 有损 | IAMF raw OBU | `.iamf` | 全平台支持；面向 IAMF 测试 / 交付，当前最高 `9.1.6` |
...
IAMF 输出为 raw OBU stream（`.iamf`）+ Opus，面向 IAMF 测试和交付链路，不是普通播放器通用容器。当前不写 22.2 / 10.2.9.3 IAMF：公开 IAMF v1.1 profile 不覆盖该布局，官方工具中的 Advanced-2 属于非公开规范路径，普通播放器也不会稳定支持。
