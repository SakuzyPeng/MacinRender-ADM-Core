...
std::filesystem::path write_fixture() {
    ...
    auto cf = adm::AudioChannelFormat::create(adm::AudioChannelFormatName{"CF"},
    ...
    auto pf = adm::AudioPackFormat::create(adm::AudioPackFormatName{"PF"},
                                           adm::TypeDefinition::OBJECTS);
    pf->addReference(cf);
    doc->add(pf);

    auto sf = adm::AudioStreamFormat::create(adm::AudioStreamFormatName{"SF"},
                                             adm::FormatDefinition::PCM);
    sf->setReference(cf);
    doc->add(sf);

    auto tf = adm::AudioTrackFormat::create(adm::AudioTrackFormatName{"TF"},
                                            adm::FormatDefinition::PCM);
    tf->setReference(sf);
    sf->addReference(tf);
    doc->add(tf);
...
int main() {
    ...
    // ── adm inspect --xml <fixture> ───────────────────────────────────────────
    {
        ...
        ok &= check(r.out.find("audioObject") != std::string::npos,
                    "inspect --xml: audioObject element");
        ok &= check(r.out.find("CliTestObj") != std::string::npos,
                    "inspect --xml: object name in XML");
    }
