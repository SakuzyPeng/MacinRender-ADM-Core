...
### Windows `win-pc` Build Notes

On `win-pc`, use the existing canonical MSVC/Ninja build path under `E:\code\MacinRender-ADM-Core`.
For a clean Windows full build, delete only `build\win-canon`, then run `E:\code\canonrun.cmd`; this
...
`E:\code\canon.flag` for `cfg=0` and `build=0`, with details in `canon-configure.log` and
`canon-build.log`.
